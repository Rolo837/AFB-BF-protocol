...
spec/
  ...
  schemas/
    ...
    tradeplan.v1.json    # шаблон торгового плана v1 (AFB-сторонний, не wire)
    tradeplan.v2.json    # шаблон торгового плана v2 (AFB-сторонний, не wire)
...
docs/PROTOCOL.md         # включая §10 про шаблоны ТП
python/                  # пакет afb-bf-protocol (модели, подпись, валидация, таксономия)
  afb_bf_protocol/schemas/  # СГЕНЕРИРОВАНО: зеркало spec/schemas/ внутри пакета
  afb_bf_protocol/payload_validation.py  # рантайм-валидация deal/tradeplan (extra [validation])
...
Для рантайм-валидации deal/tradeplan payload'ов (нужна только AFB, не BF)
добавить extra `[validation]`:

```
afb-bf-protocol[validation] @ git+https://github.com/Rolo837/AFB-BF-protocol.git@v1.0.0#subdirectory=python
```
...
- **Рантайм-валидация payload'ов** (extra `[validation]`, submodule
  `afb_bf_protocol.payload_validation`, не реэкспортируется из `__init__`,
  чтобы не тянуть `jsonschema` в базовую установку): `validate_deal`,
  `validate_tradeplan`, `resolve_tradeplan_schema`, `PayloadValidationError`.
...
> Структурную валидацию payload обеспечивают JSON-схемы в `spec/` (и, на
> рантайме, `payload_validation`). Шаблоны торговых планов (`afb.tradeplan.v1/
> v2`) описаны в `docs/PROTOCOL.md` §10 — это AFB-сторонние сущности, не
> сообщения протокола.
