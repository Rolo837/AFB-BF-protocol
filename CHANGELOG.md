...
- **Новый afb2bf-тип `calendar.update`** (`spec/schemas/payloads/calendar.update.json`, пример `examples/calendar.update.json`): replace-all снимок торгового календаря MOEX (`as_of`/`stale_after_sec`/`horizon_until` + `sections[]` по `stock`/`futures`/`currency` — `days[]` статус торгового дня (`N`/`W`/`H`, `trade_session_date`) и `windows[]` фазы сессии (`board?`/`secid?`, `start`/`end` с оффсетом, `phase`, `estimated`), разрешение окна по инструменту — board → section (уровень `secid` в схеме остался для обратной совместимости, AFB его не заполняет). `phase` — две категории: непрерывная торговля (`morning`/`main`/`evening`/`weekend`/`closing_price`) и аукцион (`opening_auction`/`closing_auction`, заявки принимаются, сделок нет) — описание поля `phase` разъясняет, кто на какую категорию гейтится (BF: вход в сделку — обе категории, watchdog/опрос брокера вне рынка — только торговля). Отправляется только BF, объявившему новый флаг `features.market_calendar` (`payloads/daemon.capabilities.json`); при устаревании снимка (старше `stale_after_sec`) BF обязан fail-safe откатиться на свой локальный конфиг. Этап 0 плана «Торговый календарь MOEX» — фундамент для последующего гейта торговли и будущих баров на графике (Этапы 1–5, отдельно).
- **Канал `market`** (`spec/schemas/afbws/market.channel.v1.json`): новый одноразовый (без подписки/пуша) `get`-таргет `calendar` (`instrument_key`, `from`, `till`) и ответ `$defs/calendar` (`afbws.market.calendar.v1`) — `windows[]` (UTC unix секунды, `phase`, `estimated`, ТОЛЬКО фазы непрерывной торговли — аукционные окна не годятся в бар графика) и `daily_bars[]` (даты дневных баров по правилу секции: у stock — все торгуемые дни включая `W`, у futures — уникальные `trade_session_date`, у currency — торговые будни) для одного инструмента, построенные AFB из того же календарного состояния, что пушится в BF через `calendar.update`. Не входит в `spec/asyncapi.yaml`, канал AFB↔BF не затрагивается. Сгенерировано: `models.ts`/`models_generated.py` (`MarketCalendar`, `MarketCalendarWindow`).
...
- **Новый канал `market`** (`spec/schemas/afbws/market.channel.v1.json`, capability `afbws.market.channel.v1`): единый schema-first канал рыночных данных взамен legacy `candles`, `stream/favorites`, `stream/positions`, `securities/marketdata|futures|positions`. Два шаблона данных — `afbws.market.series.v1` (свечи + датасеты positions/trades/hhi/orders, ключ строки `time`) и `afbws.market.snapshot.v1` (срез quote/oi/oi_daily, ключ строки `instrument_key`) — колоночные (`$defs/table`: `columns`+`rows`), без суффиксов `.request`/`.response`/`.push`: один и тот же schema id служит и ответом на `get` (есть `request_id`), и пушем по подписке (`request_id` отсутствует). Допустимые колонки и обязательная первая колонка заданы per-kind через `if`/`then`+`prefixItems`; в `series` строки датасетов существуют только на метках `candles` того же сообщения (позиции — по корзине `[t, следующая свеча)`, остальные — точным совпадением), вне торгов строк нет. Ошибки — `afbws.market.error.v1`.
  ...
  - **`afbws.market.subscribe.v1`/`afbws.market.subscription.v1`** — явные, replace-all, только для снапшот-скоупов: `quotes?: {instrument_keys}` (Избранное, maxItems 2000) и `futures?: boolean` (скринер — все фьючерсы, без списка ключей); пустое тело — отписка от обоих. `subscription` эхо принятого + `futures` (обязательное, отражает принятое состояние) + `rejected`.
  `afbws.market.snapshot.v1` несёт обязательное `scope: "quotes" | "futures" | "get"` — какой клиентский кэш заменять/обновлять: `quotes` — `full` после subscribe, затем `update` по изменившимся строкам; `futures` — всегда `full` (quote+oi+oi_daily по всем фьючерсам), после subscribe и на каждое обновление futoi (~5 мин); `get` — ответ на разовый запрос (`request_id` есть).
  Инструменты адресуются полным `instrument_key` каталога. Время получения (`received_at`) и часовой пояс рынка (`tz`) — в корне каждого сообщения с данными; `NaN` на проводе не отправляется (`null`). Не входит в `spec/asyncapi.yaml`, канал AFB↔BF не затрагивается.
- **`afbws/common.v1.json`**: новый `$defs.instrumentKey` (строка, `minLength: 1`, грамматика `<MIC>[:<board|market>]:<ticker>`, регистрозависимая); три инлайновых копии в `afbws/instrument.channel.v1.json` (`catalogAssetMember`, `inventoryListingEntry`, `assetMemberInput`) заменены на `$ref` без изменения формы на проводе.
- **`draft/interface.v1.json`**: новое опциональное `futures_days_to_expiration` (integer ≥ 0, по умолчанию 2 на фронте — как `securities.days_to_expiration` в `config/afb.yaml`) — фронт выбирает по нему основной фьючерсный контракт деривативa.
- **`market.channel.v1.json`**: `series.tables[]`/`snapshot.tables[]` теперь ссылаются на именованные `$defs.seriesTableEntry`/`$defs.snapshotTableEntry` (были анонимный inline `allOf`) — семантика валидации не изменилась, но кодогенерация теперь даёт им осмысленные имена (`MarketSeriesTable`/`MarketSnapshotTable`) вместо синтетических `Table`/`Table1`.
- **`tools/generate.py`**: новый постпроцессинг `_reorder_eager_top_level_deps()` для `models_generated.py` — датамодель-кодогенератор (`--keep-model-order`) не гарантирует, что зависимость именованного `$def`-а всегда оказывается раньше места использования; для функциональных `TypedDict("Name", {...})` (используются, когда имя поля — зарезервированное слово Python, например `from` у `MarketSeries`) это раньше приводило к `NameError` при импорте `afb_bf_protocol.models_generated` (ломало импорт всего пакета в AFB). Постпроцессинг топологически переупорядочивает верхнеуровневые операторы модуля по фактическим eager-зависимостям (базовые классы, значения присваиваний) — детерминированно и без ручной правки сгенерированного файла.
- Сгенерировано: `models.ts` (`MarketChannelV1Message` + пер-def типы, включая переименованные `MarketSeriesTable`/`MarketSnapshotTable`), `models_generated.py`, `capabilities.py`/`capabilities.ts` (`MARKET_CHANNEL_V1`).
...
— самоисцеление позиции сделки и факт исполнения в публичной проекции. Все поля новые/опциональные, провод не ломается.

- **`payloads/order.created.json`**: `order_id` теперь документирован как BF-собственный `client_order_id` (cid), а не брокерский биржевой id — старые сборки BF путали их местами, и `order.created` никогда не мэтчился с филами того же ордера. Новое опциональное поле `broker_order_id` — биржевой id отдельно, когда известен.
- **`deal_snapshot`**: описание обновлено — теперь отправляется не только по запросу `deal.operation op="reconcile"`, но и проактивно (после каждого `position.*`, в периодическом broker-reconcile проходе, после `deal.resync` при реконнекте), с дебаунсом по фингерпринту последнего отправленного снапшота. Формат сообщения не менялся.
- **`deal_positions_synced`**: помечено `deprecated` — в BF никогда не было реализовано (`grep positions_synced belphegor/` → 0 совпадений), `deal.snapshot` покрывает ту же задачу и несёт больше (`quantity`, `observed`). Оставлено в спеке только ради обратной wire-совместимости уже существующих потребителей.
- **`afbws/deal.channel.v1.json`**: новый блок `$defs/dealOpenPosition` (`qty`, `avg_price`, `as_of`), опциональное поле `position` в `dealSummary`/`dealDetail` — факт исполнения сделки (тот же источник, что у `deal.pnl.push.v1`), которого раньше не было вообще ни в одной публичной проекции сделки.
...
**Почему это PATCH, а не MAJOR, при удалении типов и полей.** `afbws/instrument.channel.v1.json` не входит в `spec/asyncapi.yaml`, не подписывается и никогда не пересекает канал AFB↔BF; обе его стороны — backend AFB и фронтенд AFB — релизятся одним артефактом и обновляются одновременно, поэтому «согласованное обновление обеих сторон» здесь не событие, а норма деплоя. Прецеденты той же квалификации: `v2.5.6` (пересборка формы `catalog`/`pool` с удалением `poolMetaResponse`/`poolSliceResponse`) и `v2.5.7` (удаление `key`/`archived` из snapshot- и upsert-форм). Коллизия в `VERSIONING.md` (§2 квалифицирует такую правку как PATCH, а строка MAJOR и §4 объявляли удаление поля MAJOR) снята — см. ниже.

**Снят прежний страж.** Тест `test_legacy_list_and_apply_are_kept_but_marked_deprecated` намеренно фиксировал решение v2.5.3 «`list`/`apply` не удалять, только пометить `DEPRECATED`». Он удалён сознательно: это обращение прежнего решения, а не подгонка под зелёное. Ту же роль теперь играет обратный страж `test_the_dead_operations_are_gone` — он следит, чтобы удалённые `$defs` не вернулись.

- **Удалены мёртвые операции**: `$defs` `listRequest`, `listResponse`, `listRequestV2`, `listResponseV2`, `listSetV2`, `listAssetV2`, `publicListing`, `applyRequest`, `applyResponse` и их ветки в корневом `oneOf`; вместе с ними — `group`, `asset`, `assets`, `catalogSet` (их читал только `list` v1 и `apply`) и `assetMember` (ни одной `$ref`). Единственный снимок каталога для любого авторизованного вызывающего — `catalog`.
- **Удалены теневые deprecated-поля**: `catalogResponse.sets` и `commitResponse.sets` (вместе с `$defs catalogSetView`), `commitRequest.sets`/`remove_sets`/`members` (вместе с `$defs setUpsert`), `$defs catalogSetEntry`. Остаются `asset_sets`/`remove_asset_sets`/`asset_set_members` и `assetSetView`/`assetSetUpsert` — по одной форме на факт. Порядок применения секций в `commitRequest.description` переписан на актуальные имена.
- **`sort_order` / `collection_sort_order` убраны с провода**. На чтении порядок несёт позиция в массиве: `collection` — позицией в `collections[]`, `assetSetView` — в `asset_sets[]`, активы внутри коллекции — порядком `assets[]` (активы одной коллекции идут подряд, коллекции — в порядке `collections[]`). У `catalogSeries` поле было мёртвым: `series` — JSON-объект (`catalogSeriesMap`), порядок членов объекта не семантичен; если порядок серий понадобится, правильный ход — перевести `series` в массив, а не возвращать поле.
- **На записи порядок переехал в секции полного порядка** — идиома уже существовавшего `membersEdit.order`: новые `commitRequest.asset_set_order` (полный порядок `set_id`) и `commitRequest.collection_order` (полный порядок `collection_id`; список плоский — в хранилище у коллекции один глобальный ключ порядка, а не позиция внутри родителя). Позиция актива внутри коллекции задаётся новой секцией `commitRequest.collection_members` (новый `$defs collectionMembersEdit`: `collection_id` + `add`/`remove`/`order` в `asset_id`; актив принадлежит ровно одной коллекции, поэтому `add` там же и переносит его из прежней, а выпавшие активы попадают в системный `_unclassified`). `sort_order` убран из `assetSetUpsert`, `collectionUpsert`, `seriesUpsert`, `collection_sort_order` — из `assetUpsert`.
- **`user`/`userState` синхронизированы с `catalog`/`commit`**: `userRequest.sets` — `assetSetUpsert` (в этой операции `visibility_tier` запрещён и отвергается `validation_error`: личные наборы имеют scope `user`); `remove_sets` → `remove_asset_sets`, `members` → `asset_set_members`. `userState.sets` — `assetSetView` с вложенным упорядоченным `asset_ids`; параллельный `userState.memberships` и `$defs setMembership` удалены. Из описаний убрано устаревшее утверждение, что операция «PHASE 3» и что backend отвечает на неё `unsupported_action` — она обслуживается.
- **Описания** (`title`/`description`) вычищены от упоминаний удалённых сущностей (`list`, `apply`, `catalogSetEntry`, `catalogSetView`, `setMembership`, `memberships`, `asset_members`, `sort_order`) — они часть контракта, из них генерится `docs/MESSAGES.md`.
- **Сгенерировано** (`afb-bf-protocol-generate`): TypeScript/Python-модели и зеркало схем `python/afb_bf_protocol/schemas/` (по нему валидирует backend AFB).
- **`python/tests/test_afbws_instrument_channel_schema.py`**: удалены тесты `list`/`apply`/`setUpsert`/`setMembership`/`assetMember`/`catalogSetEntry`/`catalogSetView` и хардкод-хэш `test_list_v2_defs_are_byte_stable_against_v2_5_5` (перехэширование удалённой проекции бессмысленно). Новые стражи: `test_every_local_ref_resolves` (ни один локальный `#/$defs/...` не ведёт на несуществующее имя — лучшая защита от полу-удаления), `test_the_dead_operations_are_gone`, `test_no_wire_entity_carries_an_order_field`, `test_commit_request_order_sections_are_full_orders`, `test_collection_members_edit_shape`, `test_user_request_speaks_the_asset_set_vocabulary`, `test_commit_request_rejects_the_removed_shadow_sections`.
- **`VERSIONING.md`**: снята коллизия уровней. В строку **PATCH** добавлена явная оговорка, что для afbws-схем, не входящих в `spec/asyncapi.yaml`, удаление и переименование — тоже PATCH; строка **MAJOR** и §4 явно ограничены типами провода `afb.execution.v1`.
- **Версии**: bump до `2.5.9` в `package.json`, `python/pyproject.toml`, `python/afb_bf_protocol/version.py`, `spec/asyncapi.yaml`; `PROTOCOL_VERSION = "afb.execution.v1"` не менялся.
