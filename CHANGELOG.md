...
- **Канал `market`** (`spec/schemas/afbws/market.channel.v1.json`): будущие (whitespace) метки баров графика теперь считает AFB на бэкенде и отдаёт прямо в `$defs/series` — новое опциональное `future_times[]` (UTC unix секунды открытия ещё не сформированных баров текущего `period` после последней свечи, по торговому календарю MOEX, строго возрастающие) на `mode:"replace"`-ответе живого `get` (target=series) и на `mode:"merge"`-пуше, когда список изменился; отсутствие поля — не менять/нет данных. Отдельного `get`-таргета `calendar` для этого больше нет — фронту он не нужен, календарь на клиент не уходит вовсе. Не входит в `spec/asyncapi.yaml`, канал AFB↔BF не затрагивается. Сгенерировано: `models.ts`/`models_generated.py` (`MarketSeries.future_times`).
