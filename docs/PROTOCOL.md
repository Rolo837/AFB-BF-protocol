...
sequenceDiagram
    ...
    note right of BF: bf_id, protocol, nonce,<br/>version, dry_run, margin_trading,<br/>heartbeat_interval_sec

    AFB->>BF: session.hello_ack
    note left of AFB: server_nonce, heartbeat_interval_sec<br/>(clamped), protocol, dry_run,<br/>margin_trading
...
|-----|-------------|-----|-----------------------|
| 1 | BF | `session.hello` | `bf_id`, `protocol`, `nonce`, `dry_run`, `margin_trading`, `heartbeat_interval_sec` |
| 2 | AFB | `session.hello_ack` | `server_nonce`, `heartbeat_interval_sec` (нормализованный), `dry_run`, `margin_trading` |
...
### 4.4 `deal.signal` — внешний сигнал на вход (DEPRECATED)

**DEPRECATED**: заменён фидом `dataset.update` (§16) — BF теперь сам
вычисляет dataset-условия по присланным AFB срезам, а не ждёт от AFB
разового сигнала о том, что условие уже выполнено. Тип **не удалён**
(удаление типа — MAJOR, см. `VERSIONING.md §4`) и по-прежнему принимается
на проводе ради обратной совместимости, но AFB его больше не отправляет.

**AFB → BF (легаси, не отправляется):**
```json
{ "deal_id": "deal-xxx:bf-id" }
```

Переводил сделку из `waiting_external_signal` → `active`.
...
| `dataset` (`positions`/`orders`/`hhi`/`trades`; `volume` — только в схеме) | `const` или `dataset` того же `dataset_id` | те же 5 скалярных | `field` **обязателен**; как индикаторы. Значения BF получает не с биржи (доступа нет), а из фида `dataset.update`, который толкает AFB (см. §16) — `current`/`previous` из последнего актуального (не stale) снапшота играют роль `cur`/`prev`; `volume` по-прежнему не вычисляется никем (AFB его не толкает, BF не ждёт) | — (не создаёт заявку напрямую) |
...
## 16. Dataset feed (`dataset.update` / `dataset.subscribe`)

### Зачем

У AFB есть платная подписка на биржевую статистику MOEX/Algopack (наборы
данных `positions`, `orders`, `hhi`, `trades` — см. §12), которую использует
`dataset`-ветка `condition.v1.json`. У BF доступа к бирже/Algopack **нет и не
будет** — токен Algopack привязан к IP AFB и не может быть роздан демонам.
Поэтому единственный способ дать BF вычислять dataset-условия самому —
AFB **пушит** демону последние срезы нужных датасетов сообщением
`dataset.update` (`system`/`afb2bf`, схема
`spec/schemas/payloads/dataset.update.json`, пример
`examples/dataset.update.json`).

Это заменяет прежнюю схему `deal.signal` (§4.4, deprecated), где AFB сам
оценивал dataset-условие и слал разовый сигнал по факту срабатывания. Теперь
условие целиком вычисляет BF (`afb_bf_protocol.condition_semantics`, как и
все прочие источники), а AFB лишь поставляет сырые данные.

### Кто определяет потребность

AFB, а не BF. Потребность в датасетах AFB выводит из условий **опубликованных
на данный BF сделок** — обходит все ноги (`entry`, `stop_loss`, `take_profit`)
и обе стороны сравнения (`left` и `right`, поскольку `right` тоже может быть
`datasetExpr` того же `dataset_id` — см. `condition.v1.json`, ветка «dataset
operator»). BF ничего не запрашивает и не подписывается сам — см.
`dataset.subscribe` ниже.

### Snapshot-семантика

`dataset.update.payload.datasets` — **полный** актуальный набор записей,
которые нужны данному BF, а не дельта. При получении сообщения BF **целиком
замещает** свой кэш этим массивом (replace-all); запись, отсутствующая в
очередном `dataset.update`, считается удалённой, а не «неизменной». Пустой
массив `datasets` — валидное сообщение, означающее «сейчас ничего не нужно»
(например, если на BF не осталось опубликованных сделок с dataset-условиями).

### Staleness

Каждая запись несёт `as_of` (ISO-время среза **на бирже**, не время отправки
AFB) и `stale_after_sec` (TTL в секундах, задаёт AFB). Запись действительна,
пока `now <= as_of + stale_after_sec`; как только это условие перестаёт
выполняться, BF обязан считать данные **отсутствующими** — dataset-условие
вычисляется в `false` (fail-safe-инвариант протокола: отсутствие данных в
любой точке вычисления → `false`, никогда исключение, см. §12). Вход по
такому условию просто не происходит; сама сделка при этом **не ставится на
паузу** и никак иначе не пенализируется — как только придёт свежий
`dataset.update` (запись снова не stale), оценка условия возобновляется
автоматически на следующем цикле, без какого-либо ручного вмешательства или
отдельного сообщения на возобновление.

### `current` / `previous` и cross-операторы

`current` — словарь «имя поля датасета → число» для текущего среза;
`previous` — тот же словарь для предыдущего среза, **опционален** (может не
быть, например сразу после подключения BF, пока AFB не накопил вторую точку).
Оба поля нужны, чтобы BF мог сыграть роль `cur`/`prev` из скалярной семантики
операторов (§12): для `above`/`below` достаточно `current`, а для
`crosses_above`/`crosses_below`/`crossing` обязательно нужен и `previous` —
без него BF не может вычислить пересечение и, по тому же fail-safe-инварианту,
условие остаётся `false`.

### Ключевание записей

Каждая запись `dataset.update` ключуется по `instrument`
(`deal.v1.json#/$defs/instrument`) — тому же инструменту, что фигурирует в
условии сделки. AFB сам резолвит биржевые особенности нейминга **до**
отправки: например, статистика по `positions` для фьючерса на MOEX ключуется
базовым активом контракта, а не кодом самого фьючерсного контракта. BF об
этой логике резолюции ничего не знает и не должен — он просто сопоставляет
записи `dataset.update` с условиями по значению `instrument`, которое видит в
обоих местах.

### Частота обновления

Сейчас MOEX republish-ит эту статистику примерно раз в 5 минут; в будущем
планируется переход на раз в минуту. BF не хардкодит никакой конкретный
период — фактический TTL каждой записи AFB сообщает через `stale_after_sec`,
так что смена биржевой частоты обновления не требует изменения протокола.

### `dataset.subscribe` — зарезервировано, не реализовано

`spec/schemas/payloads/dataset.subscribe.json` (`system`/`bf2afb`) резервирует
на будущее возможность BF запросить у AFB дополнительную подписку на датасет.
**Сейчас это НЕ реализовано ни в BF, ни в AFB**: AFB и так сам вычисляет
полный набор нужных датасетов из условий опубликованных сделок (см. выше), а
BF ничего не отправляет и не обязан уметь это делать. Фикстуры-примера для
этого типа в `examples/` намеренно нет.

## 17. Связывание ног внутри роли (`leg.logic`)

### Зачем

До этого поля единственная трактовка нескольких ног (`entry[]`, `stop_loss[]`,
`take_profit[]`) в `afb.deal.v2`/`afb.tradeplan.v2` — это **доли объёма**:
`percent` каждой ноги режет общий объём роли. Не было способа выразить «войти,
когда выполнятся **все** условия сразу» (логическое И) или «войти по **любому**
из уровней на полный объём, а не по доле» (логическое ИЛИ).

### Поле — на ноге, не на роли

Каждый элемент списка `entry`/`stop_loss`/`take_profit` (в обеих схемах —
`deal.v2.json` и `tradeplan.v2.json`) может нести опциональное поле `logic`
(`$defs.legJoin` в `deal.v2.json`, значения `split`/`and`/`or`). Это —
**инфиксный оператор связи с предыдущей ногой в том же списке**, а не режим
роли целиком. **Отсутствие поля ≡ `split`** — сегодняшнее поведение, без
исключений; старые сделки и планы, сохранённые до появления этого поля,
не мигрируют. У **первой ноги** списка `logic` не имеет смысла (связывать не
с чем) — производитель обязан её опускать или слать `split`; читатель её
игнорирует, если она всё же пришла.

### Грамматика: группы и корзины

Разбор списка ног — в две ступени, слева направо, с приоритетом **`and` выше
`or`** (`split` всегда начинает новый термин верхнего уровня):

1. **Группа** — подряд идущие ноги, связанные `and`. Оценивается как единая
   заявка на весь объём группы: гейтующие подусловия должны быть истинны
   **одновременно**, в один и тот же проход оценки условий.
2. **Корзина** — подряд идущие группы, связанные `or`. Каждая группа корзины
   остаётся независимой заявкой на рынке, но все они делят **один общий
   бюджет объёма**: как только суммарное исполнение по корзине достигает
   бюджета, оставшиеся заявки корзины снимаются с рынка.
3. `split` начинает новую корзину. Корзины делят общий объём роли между собой
   по `percent` — ровно как сегодня для одиночных ног.

```yaml
entry:
  - {condition: a}                 # начало (logic отсутствует)
  - {condition: b, logic: or}      # ИЛИ — новая группа в той же корзине
  - {condition: c, logic: and}     # И — вяжет крепче, продолжает группу b
# ⇒  a ИЛИ (b И c)
```

`percent` относится к **корзине** и осмыслен только на её первой ноге (той,
чей собственный `logic` отсутствует/`split`); на ногах, присоединённых через
`and`/`or`, `percent` **не допускается**.

Нога, за которой ничего не следует (или следующая нога снова `split`),
образует корзину из одной группы из одной ноги — поведение при этом
неотличимо от сегодняшнего, независимо от значения `logic`.

Схема **не проверяет** ни положение поля (что оно бессмысленно на первой
ноге), ни то, что производитель прислал — эти зависящие от места правила, как
и остальные позиционные ограничения этого вокабуляра, остаются на стороне
потребителя (см. BF `belphegor/protocol/validation.py`); в частности BF
сейчас дополнительно требует **однородности** значения `logic` у ног со
второй и далее в пределах одной роли (все `and`, либо все `or`, либо все
`split`/отсутствуют) — так что произвольная вложенность вида
`a ИЛИ (b И c)` протоколом уже выразима, а вот `a ИЛИ b И c` (смешение)
BF сегодня отклоняет; снятие этого ограничения не потребует изменения схемы.

### Что это НЕ меняет

Связывание ног **не влияет** на тип исполняемой заявки (LIMIT vs MARKET) —
это по-прежнему решает только оператор условия (`touch`/`above`/`below` →
лимитная, остальные → рыночная, см. §12,
`belphegor.plan_engine.order_policy`). Для группы (`and`) тип заявки
выбирается по тому же правилу применительно к единственному ценовому
подусловию группы, если оно ровно одно и несёт ненулевой уровень; при
нескольких ценовых подусловиях или их отсутствии заявка рыночная.

Это не меняет вокабуляр условий (`condition.v1.json` не тронут) и не
вводит групповые узлы условия на проводе: `entry`/`stop_loss`/`take_profit`
остаются плоскими списками ног с одиночными `event`-условиями, ровно как
сегодня. Группировка подусловий группы в единое условие оценки — деталь
исполнения на стороне BF (пакет `afb-bf-protocol` не занимается
исполнением, только описывает формат); протокол лишь передаёт список ног
и оператор связи между соседними.

### Обратная совместимость и MINOR

Поле новое, опциональное, на элементах списков в обеих схемах (`deal.v2.json`,
`tradeplan.v2.json`) — обратносовместимое расширение протокола AFB↔BF,
поэтому релиз — **MINOR**. `condition.v1.json` не меняется вообще.

## 18. Trading calendar feed (`calendar.update`)

### Зачем

BF исторически гейтит торговлю фиксированным окном времени суток и
`weekday() < 5`, поэтому дёргает брокера (свечи, tradable-check, observed
poll, watchdog) и в выходные, и ночью, и в праздники — данных о
биржевом календаре (выходные/праздничные сессии MOEX, границы фаз
торгов) у него нет и не будет: MOEX-специфичные календарные источники
(`calendars/{section}.json`, `calendars/{section}/session.json`) требуют
токена, который есть только у AFB. Поэтому, как и с dataset-фидом (§16),
AFB сам собирает календарь и **пушит** его BF сообщением `calendar.update`
(`system`/`afb2bf`, схема `spec/schemas/payloads/calendar.update.json`,
пример `examples/calendar.update.json`).

### Согласование (`features.market_calendar`)

Отправляется только BF, объявившему `daemon.capabilities.features.market_calendar: true`
(`payloads/daemon.capabilities.json`). BF, не объявивший этот флаг, никогда
не получает `calendar.update` и продолжает гейтить торговлю по своему
локальному конфигу (off-hours окно) — обратная совместимость со старыми
сборками BF не ломается.

### Snapshot-семантика

`calendar.update.payload.sections` — **полный** актуальный календарь, а не
дельта. При получении сообщения BF **целиком замещает** свой кэш календаря
этим массивом (replace-all); секция/день/окно, отсутствующие в очередном
`calendar.update`, считаются удалёнными, а не «неизменными». Горизонт
снимка (`horizon_until`) сейчас — «сегодня + 7 дней» на стороне AFB, но
задан полем, а не захардкожен на стороне BF.

### Структура снимка

- `sections[]` — по одной записи на пару (`exchange`, `section` ∈
  `stock`/`futures`/`currency`):
  - `days[]` — статус каждого календарного дня: `is_traded`, `kind`
    (`N` — обычный торговый день, `W` — сессия выходного дня, `H` —
    праздник, не торгуется), `trade_session_date` — торговая дата, в
    которую засчитывается активность этого дня (для `N` совпадает с
    `date`, для `W` — обычно следующий понедельник, для `H` — `null`);
  - `windows[]` — временные окна фаз торгов, ДВУХ категорий:
    - **непрерывная торговля** (`phase` ∈ `morning`/`main`/`evening`/
      `weekend`/`closing_price` — сделки реально идут; `closing_price` —
      синтетическая фаза, которую AFB сама достраивает на несколько минут
      после `closing_auction` до вечерней сессии, MOEX её в `session.json`
      отдельно не называет);
    - **аукцион** (`phase` ∈ `opening_auction`/`closing_auction` — период
      сбора заявок: заявки уже принимаются, но сделок ещё/уже нет).

    Неизвестная строка `phase` трактуется BF как обычная торговля
    (fail-open) — гейтить по категории имеет смысл только для двух
    перечисленных аукционных имён. `start`/`end` — ISO-время с оффсетом
    биржи. Разрешение окна по конкретному инструменту — **board → section**
    (уровень `secid` в windows остался в схеме для обратной совместимости,
    но AFB его больше не заполняет — см. `properties/windows/items/
    properties/secid`): окно с `board` действует на весь борд, окно без
    него — на всю секцию. `estimated: true` — окно спроецировано из
    выученного/дефолтного шаблона, а не взято из официального расписания
    дня биржи (MOEX отдаёт расписание сессии только на сегодня — всё, что
    дальше, обязательно проекция).

    BF использует категорию так: гейт ВХОДА в сделку (`reconcile_deal`)
    считает открытым и торговлю, и аукцион (заявку уже можно выставить в
    стакан); "тихие" проверки, которые не должна вводить в заблуждение
    неподвижная (или движущаяся без сделок) книга заявок аукциона —
    watchdog стрима, решения об опросах брокера вне рынка (`any_open`) —
    учитывают ТОЛЬКО непрерывную торговлю. AFB-сторона (`market_state`/
    `is_market_open`, будущие (whitespace) метки баров графика — `series.
    future_times`, см. `afbws/market.channel.v1.json#/$defs/series`)
    считает открытым ТОЛЬКО непрерывную торговлю — аукцион для AFB не
    торговое время.

### Staleness

Как и у dataset-фида: `as_of` + `stale_after_sec` задают TTL всего снимка
целиком (не по секциям/окнам отдельно). Пока `now <= as_of + stale_after_sec`,
снимок действителен; как только это перестаёт выполняться, BF обязан
**fail-safe** откатиться на свой локальный конфиг (off-hours окно и правило
торгового дня), а не замораживать последний известный календарь как
актуальный. Устаревший на BF снимок сигнализирует о потерянной/деградировавшей
связи AFB↔BF, а не о намеренно зафиксированном календаре.

### Дальнейшее использование (вне объёма Этапа 0)

Гейт исполнения по этому календарю в BF (`MarketHours`, приостановка опроса
брокера вне торговых окон) и вычисление `backend.is_market_open()`/
`backend.market_phase` на стороне AFB — отдельные этапы плана «Торговый
календарь MOEX» (см. `.claude/plans/`), не входят в это изменение.
